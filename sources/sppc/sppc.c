...
constexpr auto SPPC_PATH_MAX = 4096;
...
#define _thread_create_helper(MASTER, master_t, master_f, ...)                                                       \
    ...
    for (auto i = MASTER ## _CAP; i < new_cap; ++i) {                                                                \
        MASTER ## _TABLE[i].in_use = false;                                                                          \
                                                                                                                     ...
                                                                                                                     \
    MASTER ## _CAP = new_cap;                                                                                        \
    const auto id = (unsigned long)MASTER ## _CAP;                                                                   \
...
size_t fd_read(void *restrict buffer, const size_t size, const size_t count, const int fd) {
    ...
    return bytes_read < 0 ? 0 : (size_t)bytes_read / size;
}
...
size_t fd_write(void const *restrict buffer, const size_t size, const size_t count, const int fd) {
    ...
    return bytes_written < 0 ? 0 : (size_t)bytes_written / size;
}
...
size_t str_len(unsigned char const *str) {
    if (str == nullptr) { return 0; }
    return strlen((const char*)str);
}

bool  str_empty(unsigned char const *str) {
    if (str == nullptr) { return 1; }
    return strlen((const char*)str) == 0;
}

int str_cmp(unsigned char const *str1, unsigned char const *str2) {
    if (str1 == nullptr || str2 == nullptr) { return -2; }
    return strcmp((const char*)str1, (const char*)str2);
}

int str_cpy(unsigned char *restrict dest, unsigned char const *restrict src, const size_t dest_size) {
    if (dest == nullptr || src == nullptr || dest_size == 0) { return -1; }
    strncpy((char*)dest, (const char*)src, dest_size - 1);
    dest[dest_size - 1] = '\0'; // Ensure null-termination
    return 0;
}

int str_cat(unsigned char *restrict dest, unsigned char const *restrict src, const size_t dest_size) {
    if (dest == nullptr || src == nullptr || dest_size == 0) { return -1; }
    strncat((char*)dest, (const char*)src, dest_size - strlen((const char*)dest) - 1);
    dest[dest_size - 1] = '\0'; // Ensure null-termination
    return 0;
}

int str_str(unsigned char const *haystack, unsigned char const *needle) {
    if (haystack == nullptr || needle == nullptr) { return -1; }
    const auto pos = strstr((const char*)haystack, (const char*)needle);
    return pos == nullptr ? -1 : (int)(pos - (const char*)haystack);
}
...
long long net_sock_recvfrom(const int socket_fd, unsigned char *buffer, const size_t size, unsigned char *restrict host_buffer, const size_t host_size, unsigned short *restrict port) {
    ...
    if (src_addr.ss_family == AF_INET) {
        ...
        inet_ntop(AF_INET, &s->sin_addr, (char*)host_buffer, host_size);
        *port = ntohs(s->sin_port);
    }
    else if (src_addr.ss_family == AF_INET6) {
        const auto s = (struct sockaddr_in6*)&src_addr;
        inet_ntop(AF_INET6, &s->sin6_addr, (char*)host_buffer, host_size);
...
unsigned char const* proc_get_env(unsigned char const *restrict key) {
...
int proc_wait(const int pid) {
    ...
    return ret < 0 ? -1 : WEXITSTATUS(status);
}
...
int fs_symlink_target(unsigned char const *restrict path, unsigned char **restrict buffer) {
    ...
    const auto link_target = (char*)malloc(SPPC_PATH_MAX);
    if (link_target == nullptr) { return -1; }
    const auto len = readlink((const char*)path, link_target, SPPC_PATH_MAX - 1);
    if (len < 0) {
        free(link_target);
        return -1;
    }
...
pthread_t sys_thread_id() {
    return pthread_self();
}

pthread_t thread_spawn(void*(*start_routine)(void *)) {
...
int thread_join(const pthread_t thread_id) {
    if (thread_id <= 0) { return -1; }
    const auto thread = thread_id;
    return pthread_join(thread, nullptr);
}

int thread_detach(const pthread_t thread_id) {
    if (thread_id <= 0) { return -1; }
    const auto thread = thread_id;
    return pthread_detach(thread);
}

int thread_kill(const pthread_t thread_id) {
    if (thread_id <= 0) { return -1; }
    const auto thread = thread_id;
    return pthread_kill(thread, SIGKILL);
}

int thread_cancel(const pthread_t thread_id) {
    if (thread_id <= 0) { return -1; }
    const auto thread = thread_id;
    return pthread_cancel(thread);
}

int thread_equal(const pthread_t thread_id_1, const pthread_t thread_id_2) {
    if (thread_id_1 <= 0 || thread_id_2 <= 0) { return -1; }
    const auto thread1 = thread_id_1;
    const auto thread2 = thread_id_2;
